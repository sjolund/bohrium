...
cphvb_error cphvb_ve_iterator3_init(cphvb_component *self)
{
    myself = self;
...
cphvb_error cphvb_ve_iterator3_execute( cphvb_intp instruction_count, cphvb_instruction* instruction_list )
{
    cphvb_intp count;
    ...
    for (count=0; count < instruction_count; count++) {
        ...
        if ( res != CPHVB_SUCCESS ) {
            printf("Unhandled error returned by cphvb_vcache_malloc() called from cphvb_ve_iterator3_execute()\n");
            return res;
        }
        ...
        switch (inst->opcode) {                     // Dispatch instruction
            ...
            default:                            // Built-in operations
                inst->status = cphvb_compute_iterator3_apply( inst );
...
cphvb_error cphvb_ve_iterator3_shutdown( void )
{
...
cphvb_error cphvb_ve_iterator3_reg_func(char *fun, cphvb_intp *id) 
{
